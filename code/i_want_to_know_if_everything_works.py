...
w = AutomationWorkspace(workspace)
w.verification()

# 2 - raster manipulation (without AgreeDEM)
...
# 3 - raster manipulation (with AgreeDEM)
...
# 4 - catchment processing (without additional rasters)
...
# 7 - domain creation (with catchment simplification)
...
# 8 - domain creation (without catchment simplification)
domain_creation(workspace, in_dem, rise, in_selected_catchment,
                in_buildings, buffer_distance, output_folder, False)
arcpy.AddMessage("Step 7 - complete!\n")

buffer_distance = 0
# 9 - domain creation (with catchment simplification, but without buffer)
domain_creation(workspace, in_dem, rise, in_selected_catchment,
                in_buildings, buffer_distance, output_folder, True)
arcpy.AddMessage("Step 8 - complete!\n")

# 10 - export to ascii and validation
...
try:
    arcpy.Delete_management(w.path)
except:
    pass
try:
    os.remove('log')
